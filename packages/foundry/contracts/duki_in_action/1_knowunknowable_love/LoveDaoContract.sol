    ...
    IRandomiserCallbackV3
{
    ...
    function initialize(DukiDaoTypes.NetworkConfig memory config) public initializer {
        ...
        s_investment_3_fee = 1000 * DukiDaoConstants.ONE_DOLLAR_BASE;
    ...
    function connectDaoToKnow(
        ...
        if (s_community_5_Participants[msg.sender].participantNo <= 0) {
            ...
            s_community_5_Participants[msg.sender] =
                DukiDaoTypes.CommunityParticipation(s_dao_bps_count_arr[DukiDaoConstants.SEQ_5_Fire_Community_Participants], willPowerAmount, DukiDaoConstants.Initial_Evolve_Round);
        } else {
    ...
    function approveAsDukiInfluencer(address requestor) external maintainerOnly {
        addAsDaoObserver(requestor, DukiDaoConstants.SEQ_4_Thunder_DukiInfluencers, s_thunder_4_duki_influencers, DukiDaoConstants.MaxInfluencerTotal);
    }
    ...
    function connectDaoToInvest() external {
        ...
        // EFFECTS
        addAsDaoObserver(msg.sender, DukiDaoConstants.SEQ_2_Water_Investors, s_water_2_investors, DukiDaoConstants.MaxInvestorsTotal);
    ...
    function claim5Love_CommunityLotteryFairDrop() external {
        ...
        DukiDaoTypes.DaoFairDrop memory fairDrop = s_dao_fair_drop_arr[DukiDaoConstants.SEQ_5_Fire_Community_Participants];
        if (fairDrop.unitNumber <= 0) {
    ...
    function claim4Love_DukiInfluencerFairDrop() external {
        common_claim(
            DukiDaoTypes.InteractType.Out_Claim_As_DukiInfluencer, DukiDaoConstants.SEQ_4_Thunder_DukiInfluencers, s_thunder_4_duki_influencers
        );
    }

    function claim3Love_ContributorFairDrop() external {
        common_claim( DukiDaoTypes.InteractType.Out_Claim_As_Contributor, DukiDaoConstants.SEQ_3_Wind_Contributors, s_wind_3_contributors);
    }

    function claim2Love_InvestorFairDrop() external {
        common_claim( DukiDaoTypes.InteractType.Out_Claim_As_Investor, DukiDaoConstants.SEQ_2_Water_Investors, s_water_2_investors);
    }

    function claim1Love_MaintainerFairDrop() external {
        common_claim( DukiDaoTypes.InteractType.Out_Claim_As_Maintainer, DukiDaoConstants.SEQ_1_Mountain_Maintainers, s_mountain_1_maintainers);
    }

    function claim0Love_FounderFairDrop() external {
        common_claim( DukiDaoTypes.InteractType.Out_Claim_As_Founder, DukiDaoConstants.SEQ_0_Earth_Founders, s_earth_0_founders);
    }
    ...
    function common_claim(
        ...
        emit  DukiDaoTypes.DukiInAction(msg.sender, interactType, currentEvolveAge, fairDrop.unitAmount, 1, block.timestamp,"");
    }
    ...
    function commonDeductFee(DukiDaoTypes.InteractType interactType, uint256 requiredMoney) internal {
        ...
        if (success) {
            emit  DukiDaoTypes.DukiInAction(msg.sender, interactType, s_dao_evolve_round, requiredMoney, 1, block.timestamp,"");
        } else {
    ...
    function evolveDaoAndDivideLove(uint256 randomNumber) internal {
        ...
        for (uint256 i = 0; i < 8; i++) {
            uint256 bpsAmount = (s_dao_bps_arr[i] * distributionAmount) / DukiDaoConstants.BPS_PRECISION;
            ...
            if (DukiDaoConstants.SEQ_7_Heaven_ALM_DukiInAction == i) {
                ...
                daoFairDrops[i] = DukiDaoTypes.DaoFairDrop(DukiDaoConstants.DukiInAction_StableCoin_Claim_Amount, almUnitTotalNum, almUnitTotalNum);
